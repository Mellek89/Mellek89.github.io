...
if (ehrenEl.classList.contains("active")) {
  ...
  // Letzte Tage vom Vormonat anzeigen
  for (let i = firstDateOfMonth; i > 0; i--) {
    const inactiveDay = lastDateOfLastMonth - i + 1;
    liTag += `<li class="inactive">${inactiveDay}</li>`;
  }

  // Aktuelle Tage des Monats
  for (let i = 1; i <= lastDateOfMonth; i++) {
    ...
    // Priorität 2: Events mit Zeitspanne
    else if (eventId !== "" && daysOfEvents.length > 0) {
      const monthIndex = currMonth + 1;

      if (
        // One-time Event (Tag, Monat)
        (daysOfEvents.length === 2 &&
          i === daysOfEvents[0] &&
          monthIndex === daysOfEvents[1]) ||

        // Multi-month Event (vonTag, vonMonat, bisTag, bisMonat)
        (daysOfEvents.length === 4 &&
          (
            (i >= daysOfEvents[0] && monthIndex === daysOfEvents[1]) ||
            (i <= daysOfEvents[2] && monthIndex === daysOfEvents[3]) ||
            (monthIndex > daysOfEvents[1] && monthIndex < daysOfEvents[3])
          )
        )
      ) {
        className = "circle";
  ...
  // Erste Tage vom nächsten Monat anzeigen
  for (let i = lastDayOfMonth; i < 6; i++) {
    const nextMonthDay = i - lastDayOfMonth + 1;
    liTag += `<li class="inactive">${nextMonthDay}</li>`;
  }

  // Setze Datum + Tage in DOM
  currentDate.innerText = `${months[currMonth]} ${currYear}`;
  daysTag.innerHTML = liTag;

  // Leere das Array für wiederkehrende Wochentage im aktuellen Monat
  recurringWedOfCurrentMonth.length = 0;
  recurringThuOfCurrentMonth.length = 0;
};
...
function setActiveMarket(market) {
  ...
  renderCalender(); // deine vorhandene Funktion
...
renderCalender();
document.addEventListener("DOMContentLoaded", function () {
    const ehrenEl = document.getElementById("Ehrenbreitsteiner-Wochenmarkt");
    const seltersEl = document.getElementById("Selters-Wochenmarkt");
